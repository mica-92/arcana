...
// ACTUALIZAR la función calculateBasicStats para incluir signos
function calculateBasicStats(entries) {
    ...
    dailyEntries.forEach(entry => {
        if (entry.tarot_card) {
            ...
            // Detectar si es arcano mayor (compatible con "Majors" y "Major Arcana")
            const isMajorArcana = cardSuit === 'Major Arcana' || cardSuit === 'Majors';
            
            // CLASIFICACIÓN PRINCIPAL - solo una categoría por carta
            if (isCourtCard) {
                ...
                switch(cardSuit) {
                    ...
                    case 'Pentacles':
                        ...
                        break;
                    default:
                        // Si no se clasificó, contar como Major Arcana por defecto
                        statisticsData.suits['Major Arcana']++;
                }
            }
            
            // ELEMENTOS - incluyendo cartas de corte con doble elemento
            if (!isCourtCard) {
                // Lógica actual para no court cards
                ...
                // NUEVA LÓGICA PARA COURT CARDS - contar ambos elementos
                const cardName = card.Name || '';
            ...
            // PLANETAS - solo para no court cards y si tiene planeta
            if (!isCourtCard && card.Planet && statisticsData.planets.hasOwnProperty(card.Planet)) {
                statisticsData.planets[card.Planet]++;
            }
            
            // SIGNOS ZODIACALES - solo para no court cards y si tiene signo
